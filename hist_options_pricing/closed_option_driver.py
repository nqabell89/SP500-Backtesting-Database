# Description: This script is the sister script to driver_data.py and works for options that have already expired. The catch is that you need to already know all the info about the options since we can't fetch a list of past expiry dates or strikes available on a given date.
...
if (symbol.lower() == "settings"): # Does the user want to change the settings
    settings = data_grab.modify_settings(settings)    
    symbol = input("Enter a symbol to proceed: ").upper(); check_sentinel(symbol)


description = data_grab.background_info(symbol, API_KEY) # Display some info about the underlying
...
startDate, should_use_history_endpoint = data_grab.get_start_date(int(settings['historyLimit']))
...
trade_data = data_grab.get_trade_data(option_symbol, startDate, settings['binning'], should_use_history_endpoint, API_KEY)
